...
func NewClickhouseExecutor(datasource *models.DataSource) (tsdb.Executor, error) {
...
func init() {
	glog = log.New("tsdb.clickhouse")
	tsdb.RegisterExecutor("clickhouse", NewClickhouseExecutor)
}

func (e *ClickhouseExecutor) Execute(ctx context.Context, queries tsdb.QuerySlice, context *tsdb.QueryContext) *tsdb.BatchResult {
	result := &tsdb.BatchResult{}
	params := url.Values{}
	params["query"] = []string{queries[0].Model.Get("compiledQuery").MustString() + " FORMAT JSON"}

	if setting.Env == setting.DEV {
		glog.Debug("Clickhouse request", "params", params)
	}

	req, err := e.createRequest(params)
	if err != nil {
	...
	if err != nil {
		glog.Info("Failed to conert respone", "error", err)
		return result.WithError(err)
	}
...
func (e *ClickhouseExecutor) createRequest(data url.Values) (*http.Request, error) {
	u, _ := url.Parse(e.Url)
...
func (e *ClickhouseExecutor) parseResponse(res *http.Response) (*TargetResponseDTO, error) {
	...
	if err != nil {
		glog.Info("Failed to unmarshal clickhouse response", "error", err, "status", res.Status, "body", string(body))
		return nil, err
	}
